...
public class Connect {
  ...
  static void present( String[] board ) {
    for ( int row = ROW_SIZE - 1; row >= 0; row -- ) {
      for ( int col = 0; col < COL_SIZE; col ++ ) {
        ...
        System.out.print( COLOR_PIECE[ getPiece( board, col, row ) ] );
      }
      System.out.println();
    }
    System.out.println( "-------" );
   ...
   * 4. Obtain the row number
   * 5. Place the piece for the player
   * 6. Evaluate and report if the player has won
   */
  static boolean playerZero( String[] board, int player ) {
    int col, row;
